...
test("Get all reviews of a member", async () => {
...
test("Get all reviews of non-existing member", async () => {
  expect.assertions(1);
  const review = await functions.readMemberReviews("1234").catch(error => {
    expect(error.response.status).toEqual(400);
  });
}, 50000);
test("Get a non-existing review", async () => {
  expect.assertions(1);
...
test("Get a single review", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
...
test("Create a review with a non-existing member", async () => {
  expect.assertions(1);
  const partners = await partnerFunctions.getPartners();
  if (partners.data.data.length > 0) {
    const partner = partners.data.data[0];
...
test("Create a review with a non-existing partner", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
    const member = members.data.data[0];
...
test("Create a review with incorrect or incomplete data", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
    const member = members.data.data[0];
...
test("Update a review", async () => {
  ...
  if (members.data.data.length > 0) {
    ...
    console.log(memberId, reviewId, body, "HOBA");
    await functions.updateReview(memberId, reviewId, body);
    const tempReview = await functions.readReview(reviewId);
    console.log(tempReview.data.data, "TEMP");
    expect(tempReview.data.data).toEqual(expect.objectContaining({ ...body }));
  } else expect(members.data.data).toEqual([]);
}, 50000);

test("Update a non-existing review", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
    const member = members.data.data[0];
...
test("Update a review in non-existing member", async () => {
  expect.assertions(1);
  const memberId = "1234";
  const reviewId = "1234";
  const body = {
...
test("Update a review with missing or incorrect info", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
    const member = members.data.data[0];
...
test("Delete a non-existing review for existing member", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
    const memberId = members.data.data[0]._id;
...
test("Delete an  existing review for non-existing member", async () => {
  expect.assertions(1);
  const members = await memberFunctions.getMembers();
  if (members.data.data.length > 0) {
