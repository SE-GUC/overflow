...
test("Get lifeCoach slots", async () => {
  expect.assertions(1);
...
test("Get slots of a non existent partner", async () => {
  expect.assertions(1);
...
test("Get a non existent slot", async () => {
  expect.assertions(1);
...
test("Create a new slot with no member", async () => {
  const lifeCoaches = await getLifeCoaches();
...
test("Create a new slot with a member", async () => {
  const lifeCoaches = await getLifeCoaches();
...
test("Creating a slot with a non existent member", async () => {
  const lifeCoaches = await getLifeCoaches();
  if (lifeCoaches.data.data.length > 0) {
    expect.assertions(1);
    const lifeCoach = lifeCoaches.data.data[0];
...
test("Create a slot with incomplete or incorrect data", async () => {
  expect.assertions(1);
...
test("Update a slot with incomplete or incorrect data", async () => {
  expect.assertions(1);
  const lifeCoachRequest = await getLifeCoaches();
  const { data } = lifeCoachRequest.data;
  const lifeCoach = data.find(
...
test("Update a non existent slot in a non existent lifeCoach", async () => {
  expect.assertions(1);
  const req = {
    booked: true,
    confirmed: false,
...
test("Update a non existent slot in a lifeCoach", async () => {
  expect.assertions(1);
  const lifeCoaches = await getLifeCoaches();
  const { data } = lifeCoaches.data;
  if (data.length > 0) {
...
test("Delete a slot in a non existent lifeCoach ", async () => {
  expect.assertions(1);
  const req = {
    booked: true,
    confirmed: false,
...
test("Delete a non existent slot in a lifeCoach", async () => {
  expect.assertions(1);
  const lifeCoaches = await getLifeCoaches();
  const { data } = lifeCoaches.data;
  if (data.length > 0) {
