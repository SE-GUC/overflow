...
test(
  "Get all vacancies of a non existent partner",
  async () => {
    expect.assertions(1);
    const vacancies = await functions
      .getPartnerVacancies("1234")
      .catch(error => {
...
test(
  "Create a vacancy with incomplete or incorrect data",
  async () => {
    expect.assertions(1);
...
test(
  "Update a vacancy with incomplete or incorrect data",
  async () => {
    expect.assertions(1);
    const vacancies = await functions.getVacancies();
    if (vacancies.data.data.length > 0) {
